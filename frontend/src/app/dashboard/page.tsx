    ...
    <div className="min-h-screen bg-orange-50/40">
      <nav className="bg-gray-950 border-b border-gray-800 px-6 py-3 flex items-center justify-between sticky top-0 z-30">
        <div className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            <span className="font-semibold text-white block">Panel de Selección</span>
            <span className="text-[11px] text-orange-400/80 block">San Buenaventura Bogotá</span>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="text-sm bg-orange-500 hover:bg-orange-400 text-gray-950 px-3 py-1.5 rounded-lg font-semibold transition-colors"
          >
            + Nueva vacante
          </button>
          <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total candidatos', value: stats.total, color: 'text-gray-900', accent: 'border-l-gray-900' },
            { label: 'Preseleccionados', value: stats.preseleccionados, color: 'text-emerald-600', accent: 'border-l-emerald-500' },
            { label: 'Por revisar', value: stats.analizados, color: 'text-orange-600', accent: 'border-l-orange-500' },
            { label: 'Descartados', value: stats.descartados, color: 'text-red-500', accent: 'border-l-red-400' },
          ].map(stat => (
            <div key={stat.label} className={`bg-white rounded-xl border border-gray-100 border-l-4 ${stat.accent} p-5 shadow-sm`}>
          ...
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors
                ${pestana === p.id ? 'bg-gray-950 text-orange-400 shadow-sm' : 'text-gray-500 hover:text-gray-900'}`}
            >
              {p.label}
            </button>
          ))}
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-4 mb-3 space-y-3 shadow-sm">
              ...
              {hayFiltrosActivos && (
                <div className="text-xs text-gray-500 pt-1">
                  Mostrando <span className="font-semibold text-orange-600">{candidatosFiltrados.length}</span> de {candidatos.length} candidatos
                </div>
              )}
            </div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-950 border-b border-gray-800">
                      <tr>
                        ...
                        <th className="text-left px-4 py-3 font-medium text-gray-300">Vacante</th>
                        <th className="text-left px-4 py-3 font-medium text-gray-300">Estado</th>
                        <th className="text-center px-4 py-3 font-medium text-gray-300">Score</th>
                        ...
                        <th className="text-center px-4 py-3 font-medium text-gray-300">Acción</th>
                      </tr>
                    </thead>
                        ...
                        const scoreColor = score === null ? 'text-gray-400'
                          : score >= 80 ? 'text-emerald-600'
                            : score >= 60 ? 'text-orange-500'
                              : 'text-red-500'
                        const recomColor = ev?.recomendacion === 'contratar' ? 'bg-emerald-100 text-emerald-700'
                          : ev?.recomendacion === 'entrevistar' ? 'bg-orange-100 text-orange-700'
                            : ev?.recomendacion === 'descartar' ? 'bg-red-100 text-red-700'
                              : 'bg-gray-100 text-gray-500'
                        return (
                          <tr key={c.id} className="hover:bg-orange-50/60 transition-colors">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-gray-950 border-b border-gray-800">
                    <tr>
                      <th className="text-left px-4 py-3 font-medium text-gray-300">Vacante</th>
                      <th className="text-left px-4 py-3 font-medium text-gray-300">Departamento</th>
                      <th className="text-left px-4 py-3 font-medium text-gray-300">Modalidad</th>
                      <th className="text-left px-4 py-3 font-medium text-gray-300">Estado</th>
                      <th className="text-center px-4 py-3 font-medium text-gray-300">Candidatos</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {vacantes.map(v => {
                      ...
                      const estadoColor = v.estado === 'activa' ? 'bg-emerald-100 text-emerald-700'
                        : v.estado === 'pausada' ? 'bg-amber-100 text-amber-700'
                          : 'bg-red-100 text-red-700'
                      return (
                        <tr key={v.id} className="hover:bg-orange-50/60 transition-colors">
                          <td className="px-4 py-3">
        ...
        <form onSubmit={handleGuardar} className="p-6 space-y-5">
          <div>
            ...
            <input name="titulo" required value={form.titulo} onChange={handleChange}
              ...
              className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-orange-400" />
          </div>
          <div>
            ...
            <textarea name="descripcion" required value={form.descripcion} onChange={handleChange} rows={3}
              ...
              className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-orange-400 resize-none" />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={guardando}
              className="flex-1 px-4 py-2.5 bg-gray-950 hover:bg-black disabled:bg-gray-400 text-orange-400 rounded-lg text-sm font-semibold transition-colors flex items-center justify-center gap-2">
              {guardando ? (
                <>
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          {ev ? (
            <div className="bg-gray-950 rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-sm font-semibold text-orange-400">Score total IA</h3>
                  <p className="text-xs text-gray-400 mt-0.5">
                    Recomendación:{' '}
                    <span className={`font-semibold capitalize
                      ${ev.recomendacion === 'contratar' ? 'text-emerald-400'
                        : ev.recomendacion === 'descartar' ? 'text-red-400'
                          : 'text-orange-300'}`}>
                      {ev.recomendacion}
                    </span>
                  </p>
                ...
                <div className={`text-4xl font-bold
                  ${Number(ev.score_total) >= 80 ? 'text-emerald-400'
                    : Number(ev.score_total) >= 60 ? 'text-orange-400'
                      : 'text-red-400'}`}>
              ...
              <ScoreBar label="Fit cultural (10%)" value={Number(ev.score_fit_cultural)} color="bg-orange-400" />
            </div>
          ) : (
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {ev.habilidades_detectadas?.map((h, i) => (
                    <span key={i} className="px-2.5 py-1 bg-orange-50 text-orange-700 rounded-full text-xs">{h}</span>
                  ))}
                </div>
              </div>
          ...
          <div className="border-t border-gray-100 pt-5">
            ...
            <textarea
              ...
              className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-900
                         bg-white focus:outline-none focus:ring-2 focus:ring-orange-400 resize-none"
            />
            <div className="flex items-center justify-between mt-2">
              ...
              <button
                ...
                className="px-4 py-1.5 bg-gray-950 hover:bg-black disabled:bg-gray-400
                           text-orange-400 rounded-lg text-xs font-semibold transition-colors"
              >
          ...
          <div className="border-t border-gray-100 pt-5">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                { estado: 'preseleccionado', label: '✓ Preseleccionar', color: 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200' },
